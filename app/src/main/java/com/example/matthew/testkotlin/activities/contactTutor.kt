...
import kotlinx.android.synthetic.main.activity_contact_tutor.*


class contactTutor : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnCAT.setOnClickListener({
            if(name.text.toString().length>0 && email.text.toString().length>0 && level.text.toString().length==2 && deptid.text.toString().length>0 && phoneNumber.text.toString().length>10 && facultyid.text.toString().length>0 && courseid.text.toString().length>0){
                ...
                db.insertTData(info)
                val myToast = Toast.makeText(this, "We will get a tutor to you soon", Toast.LENGTH_SHORT)
                myToast.show()
                ...
                Toast.makeText(this, "Fill in All Allocations to get a Tutor...", Toast.LENGTH_SHORT).show()
            }
